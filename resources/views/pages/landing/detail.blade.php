        ...
        <div class="grid gap-5 md:grid-cols-12">
            <main class="p-4 lg:col-span-8 md:col-span-12">
                ...
                @if (count($thumbnail) > 0)
                    ...
                    @else
                        <div class="p-3 mt-4 bg-gray-100 rounded-lg image-gallery" x-data="gallery()">
                @endif
        ...
        <div class="content">
            <div x-data="{ tab: window.location.hash ? window.location.hash.substring(1) : 'description' }" id="tab_wrapper">
                        ...
                        <div class="flex items-center col-span-12 p-2 lg:col-span-6">
                            <div class="flex items-center space-x-4">
                                @if ($service->user->detail_user->photo != null)
                                    <img src="{{ url(Storage::url($service->user->detail_user->photo)) }}"
                                        alt="photo profile" class="w-20 h-20 object-cover rounded-full " loading="lazy">
                                @else
                ...
                <div x-show.transition.duration.500ms="tab === 'reviews'">
                    <h2 class="mb-4 text-xl font-semibold"><span class="text-serv-button">{{ sizeof($review) }}</span>
                        Total Reviews</h2>
                            ...
                            <div class="flex items-start px-4 py-6">
                                @if ($item->user_buyer->detail_user->photo != null)
                                    <img class="w-16 h-16 rounded-full object-cover mr-4"
                                        ...
                                        alt="photo profile">
                                @else
                                    <svg class="w-16 h-16 rounded-full object-cover mr-4 text-gray-300" fill="currentColor"
                                        viewBox="0 0 24 24">
                                        <path
                                            d="M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" />
                                    </svg>
                                ...
                                <div class="w-full">
                                    ...
                                    <p class="text-md">
                                        ...
                                        @endfor
                                        <span
                                            class="text-serv-yellow font-medium mt-2 inline align-sub">{{ $item->rating }}</span>
                                    </p>
        ...
        <aside class="p-4 lg:col-span-4 md:col-span-12 md:pt-0">
            <div class="mb-4 border rounded-lg border-serv-testimonial-border">
                ...
                <div class="flex items-start px-4 pt-6">
                    @if ($service->user->detail_user->photo != null)
                        <img src="{{ url(Storage::url($service->user->detail_user->photo)) }}" alt="photo profile"
                            class="object-cover w-16 h-16 mr-4 rounded-full" loading="lazy">
                    @else
