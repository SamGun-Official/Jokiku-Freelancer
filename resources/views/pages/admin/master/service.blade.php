            ...
            <div class="grid w-full gap-5 px-10 mx-auto md:grid-cols-12">
                <div class="col-span-8">
                    ...
                    <p class="text-sm text-gray-400">
                        {{ $services->count() }} Total Pending Services
                    </p>
                </div>
                            ...
                            <tbody class="bg-white">
                                @forelse ($services as $key => $item)
                                    <tr class="text-gray-700 border-b">
                                                ...
                                                <div class="relative w-10 h-10 mr-3 rounded-full md:block">
                                                    @if (isset($item->thumbnail_service[0]->thumbnail) != null)
                                                        <img class="object-cover w-full h-full rounded"
                                                            src="{{ url(Storage::url($item->thumbnail_service[0]->thumbnail)) }}"
                                        ...
                                        <td class="px-1 py-5 text-sm">
                                            @if ($item->status == 0)
                                                <a href="{{ route('admin.approve.service', $item['id']) }}"
                                                    class="px-4 py-2 mt-2 text-left text-white rounded-xl bg-green-500">
                                                    Approve
                                                </a>
                                                <a href="{{ route('admin.reject.service', $item['id']) }}"
                                                    class="px-4 py-2 mt-2 text-left text-white rounded-xl bg-red-500"
